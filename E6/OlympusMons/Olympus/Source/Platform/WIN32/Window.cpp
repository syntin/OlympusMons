...
namespace Win32 {
	...
	Window::Window(WSTRING title, HICON icon, WindowType type)
		: Win32::SubObject(title, title, icon), _type(type)
	{
	...
	VOID Window::Initialize()
	{
		...
		AdjustWindowRect(&R, _type, false);
		int width = R.right - R.left;
		int height = R.bottom - R.top;

		_handle = CreateWindow(_wClass.c_str(), _title.c_str(),
			_type, ((desktop.right / 2) - (Size().cx / 2)), ((desktop.bottom / 2) - (Size().cy / 2)), Size().cx, Size().cy, 0, 0, HInstance(), (void*)this);
	...
	VOID Window::PaintCaption(HDC hdc)
	{
		...
		if (ShowTitle())
		{
			...
			DrawText(hdc, _title.c_str(), static_cast<int>(wcslen(_title.c_str())), &rect, DT_SINGLELINE | DT_VCENTER | DT_CENTER);
		}
		...
		{;
			...
			button->Rect = RECT{ _size.cx - spacing , 0, _size.cx - spacing + button->Width , 30 };
			...
			DrawText(hdc, button->Text.c_str(), static_cast<int>(wcslen(button->Text.c_str())), &button->Rect, DT_SINGLELINE | DT_VCENTER | DT_CENTER);
		}
