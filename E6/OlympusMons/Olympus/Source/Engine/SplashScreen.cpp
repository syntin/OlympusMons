...
namespace SplashScreen {
	...
	{
		PostMessage(splashWindow->Handle(), WM_OUTPUTMESSAGE, (WPARAM)message, 0);
	}
}


SplashWindow::SplashWindow()
	: Win32::Window(L"SplashScreen", NULL)
{
...
LRESULT SplashWindow::MessageHandler(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		case WM_PAINT:
		{
			...
			TextOut(hdc, Size().cx / 2, Size().cy - 30, _outputMessage, static_cast<int>(wcslen(_outputMessage)));
			EndPaint(hwnd, &ps);
		}
		break;
